...
## 다음 단계: 0.2.0 배포 및 업데이터 교체

- [x] 기존 커스텀 업데이터 모듈(`rust/overmax_app/src/updater`) 제거
- [x] `self_update` 크레이트 도입 및 연동 (`Cargo.toml`에 추가)
- [x] 포터블 환경 유지를 위한 `.exe` 파일 내부 교체(`self_replace`) 로직 검증
- [x] 0.2.0 버전용 GitHub Releases 연동 테스트
...
- Rust backend는 검증 스크립체에서 충분히 확인된 뒤 프로덕션 검색 경로에 연결한다.
