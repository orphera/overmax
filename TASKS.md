...
- [x] `infra/gui/controls.py` 신규 생성: 공용 컨트롤 생성 헬퍼 이동
- [x] `infra/gui/layout.py` 신규 생성: `LayoutContext`, `LayoutPadding` 이동
- [x] `infra/gui/dpi.py` 보강: `scale_for_dpi`, `scaled_value` 이동
- [x] `infra/gui/placement.py` 보강: `center_position` 이동
- [x] `infra/gui/theme.py` 보강: `create_font` 이동 및 기본 색상 토큰 추가
- [x] `overlay/win32/settings_common.py` 제거 및 관련 창 임포트 갱신
- [x] 모든 Win32 smoke 테스트 통과 확인 (Settings, Sync)

---

## Phase 14: Qt Removal (Completed)

PyQt6 의존성을 완전히 제거하고 네이티브 Win32 인프라로 전환을 완료했다.

- [x] `PyQt6` 의존성 완전 제거 (`requirements.txt`)
- [x] 레거시 Qt 컴포넌트 삭제 (`overlay/ui/`, `tray_icon.py` 등)
- [x] 프로젝트 공용 네이티브 시그널 시스템 도입 (`infra/gui/signals.py`)
- [x] Win32 네이티브 트레이 아이콘 구현 (`infra/gui/tray.py`)
- [x] `OverlayController` Win32 전용으로 리팩토링
