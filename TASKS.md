...
## 7. OBS 방송 송출용 데이터 서빙 (OBS Integration)

- [ ] 로컬 HTTP / WebSocket 서버 연동을 통한 오버레이 실시간 상태(JSON) API 서빙
- [ ] OBS 브라우저 소스(Browser Source)용 경량 알파 투명 HTML/JS 템플릿 설계
- [ ] 방송 송출 시 UI 인터랙션 요소 배제 및 순수 정보 디스플레이 레이아웃 바인딩
...
- [ ] `orphera/overmax-image-db` 리포지토리 재작업을 통한 전체 자켓 이미지의 HOG 피처 일괄 갱신 및 재빌드
  ...
  - [ ] 특정 자켓(Fundamental 등)의 이미지 정규화(Contrast Normalization) 혹은 개별 매칭 가중치 보정 설계
  - [ ] 정식 배포 시 글로벌 자켓 매칭 기본 임계치(`settings.json`) 검증 및 0.80~0.85 구간 최적값 확정
...
## 11. HOG 자켓 이미지 매칭 성능 최적화 (백로그)

- [ ] 투스텝(Two-step) 자켓 검색 파이프라인 설계 (완료)
  - [x] 연산량이 적은 Perceptual Hash(pHash) 등으로 매칭 후보군을 상위 K개(예: 5~10개)로 1차 필터링하고 모호할 때만 HOG 연산 적용 (하이브리드 매쳐 연동 완료)
- [ ] 매칭 씬 캐시 레이어 도입
  - [ ] 최근 매칭된 곡 혹은 선곡 빈도가 높은 곡 정보를 로컬 캐시하여 중복 검색 생략
...
- [ ] 캡처 및 디텍션 제어 루프 최적화
  ...
  - [ ] 선곡 화면 확정 시 자켓 영역만 부분 BitBlt 캡처하도록 제어 루프 스위칭 (옵션 B 백로그)
  - [ ] DXGI 백엔드 구동 시 GPU-side 복사(CopySubresourceRegion)를 통해 자켓/로고 영역만 CPU로 스테이징 맵핑하여 memcpy 원천 차단 (옵션 C 백로그)
  - [ ] `AdaptiveCaptureEngine` 캡처 FPS 제한 및 스레드 적정 sleep 제어 보완
  - [ ] 변화량 감지 캐시를 통한 무의미한 프레임 스킵 (화면 픽셀 변화가 없을 시 HOG 매칭 및 OCR 연산 생략 조기 리턴)
- [ ] 그래픽 및 GUI 렌더링 최적화
  - [ ] egui 이벤트 루프에서 불필요한 `request_repaint` 스팸 호출 조사 및 억제
