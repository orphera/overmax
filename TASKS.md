...
- [ ] HOG 피처 매칭 연산 속도 개선 및 최적화
  - [ ] HOG 특징량 비교 연산에 SIMD 명령어셋(AVX2 등) 도입 또는 병렬 연산(Rayon) 적용
  - [ ] 피처 벡터 차원 및 셀 크기 튜닝을 통한 특징량 비교 오버헤드 축소
- [ ] 투스텝(Two-step) 자켓 검색 파이프라인 설계
  - [ ] 연산량이 적은 Perceptual Hash(pHash) 등으로 매칭 후보군을 상위 K개(예: 5~10개)로 1차 필터링
  - [ ] 필터링된 초소형 후보군에 대해서만 HOG 정밀 매칭을 수행하여 자켓 인식 프레임 시간 300ms -> 10ms 이하로 단축
- [ ] 자켓 매칭 캐시 레이어 도입
  - [ ] 최근 매칭된 곡 혹은 선곡 빈도가 높은 곡 정보를 로컬 캐시하여 검색 생략 유도
