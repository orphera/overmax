...
- `main`과 현재 HEAD의 merge-base를 구하고, merge-base 대비 현재 branch diff를 수집해 이번 기능의 변경과 선행 Windows/atlas 작업을 구분한다.
- `pr-27` 대비 현재 브랜치의 변경 파일 및 씬 관련 변경을 확인한다.
- PR에서 추가된 코드와 기존 main 공통 조상의 대응 경로를 확인해, 기존 구현을 확장하는 편이 자연스러운 부분을 정리한다.
- 다음 전이에 대한 기대 입력/출력 표를 만든다: 선곡→Gameplay, Gameplay→Paused→Gameplay, Gameplay→결과, 결과→선곡, Gameplay 판독 miss, atlas/full-frame 전환, 창/포커스/캡처 reset.
- 기존 테스트 중 단순히 내부 함수를 직접 호출하는 테스트와 실제 `detect()` 입력 경로를 타는 테스트를 구분한다.
- **완료 기준:** 기존 parser/확정/기록 경로와 현재 Gameplay 동작을 독립적으로 검증할 최소 회귀 테스트 목록이 확정된다.

### 2단계: 캡처 및 atlas 계약 추적

- `CapturedFrame` 생성 지점과 Windows GDI/DXGI worker 분기를 추적한다.
- atlas 슬롯 생성·복사·translator 경로가 Gameplay ROI에 대해 실제 런타임까지 이어지는지 확인한다.
- Linux 및 full-frame 경로에서 `GameplaySceneReader`가 받는 데이터의 좌표·크기·stride 계약을 확인한다.
- **완료 기준:** 지원 프레임 형태별 실제 호출 흐름을 코드 위치와 테스트로 설명할 수 있다. 미확인 영역은 구현 가정으로 채우지 않는다.

### 3단계: 최소 통합 설계 선택

다음 후보를 실제 계약과 비용을 비교해 선택한다.

- **A — 공용 후보 수집:** 두 판독기가 후보를 산출하고 단일 함수가 후보 선택/충돌 해소 후 공용 commit에 전달.
- **B — 기존 parser에 판독기 통합:** 정적 parser의 반환 계약을 확장해 Gameplay/Paused 후보도 같은 parse 결과로 전달. parser의 부수 효과와 비용을 감안해 경계가 더 단순해지는 경우에만 선택.
- **C — 현 구조 유지:** 별도 `InGame` 분기가 정적 parser 생략 등 검증 가능한 요구를 충족한다면 근거와 테스트를 보강해 유지.

- 어느 선택에서도 reader 안에 history/cooldown/상태 commit을 넣지 않는다.
- 후보 계약과 상태 전이는 기존 타입/helper로 표현 가능한지 먼저 확인한다. 새 타입이나 상태 필드는 기존 구현으로 불가능한 경우에만 허용하고 근거를 남긴다.
- **완료 기준:** 선택 근거, 거부한 대안, 비용/정확도 영향, 후보 충돌 동작이 계획에 기록된다.

### 4단계: 검증 우선 리팩터링

- 먼저 공용 관측/후보 선택 테스트를 작성한다: Gameplay와 정적 후보의 동시 관측, 후보 miss, 서로 다른 연속 후보, cached tick, 후보→commit 출력.
- 정적 결과 씬 테스트 및 기존 verified event/record 테스트를 변경 전후 동일하게 유지한다.
- 테스트가 규정한 공용 경로로 최소 변경한다. atlas packing이나 무관한 translator 정리는 하지 않는다.
- 기능별 불필요 코드 제거 후 `merge-base(main, HEAD)` 기준 diff를 다시 검토한다. PR 원본에 있던 코드라도 현재 목표에 불필요하면 제거할 수 있고, 선행 Windows 지원 변경은 별도 근거 없이 제거하지 않는다.
- PR 추가 helper/state/type를 기존 구조와 함께 검토해 중복은 통합하고, 별도 요소가 남는 경우 역할이 드러나도록 정리한다.
- **완료 기준:** 새 후보 흐름과 기존 경로 회귀 테스트가 통과한다. 성능 기준 변화가 생기면 근거 없는 최적화 주장을 하지 않는다.

### 5단계: Windows 및 전체 검증

- workspace의 기존 Windows/Linux 빌드·테스트 명령과 관련 Clippy를 실행한다.
- 가능한 경우 Windows GDI 및 DXGI 캡처 각각에서 동일 상태 전이 시나리오를 확인하고, 실측 불가능하면 해당 항목을 미검증으로 남긴다.
- atlas와 full-frame 결과를 같은 픽셀 fixture로 대조한다.
- **완료 기준:** 자동 검증 결과와 실앱 검증 범위를 구분해 기록하고, 실패/미검증을 명시한다.

### 6단계: 문서 정리

- 재설계가 기존 계약/제약을 바꾸는 경우에만 `CONTEXT.md`와 해당 decision log를 갱신한다.
- 작업 계획의 완료 표시는 실제 검증 완료 이후에만 갱신한다.
