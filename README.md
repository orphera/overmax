...
| 플레이 상태 감지 | `PlayStateDetector` — 모드/난이도/Max Combo/Rate 통합 감지 |
| Rate 자동 수집 | Windows OCR → RecordDB (SQLite) 자동 저장 (중복 수집 방지) |
| Max Combo 감지 | 뱃지 영역 밝기 기반 감지 |
| V-Archive 연동 | API 기반 유저 기록 불러오기 + 갱신 후보 추출 및 등록 |
...
│   ├── hysteresis.py            # 히스테리시스 버퍼 (선곡화면 진입/이탈 판정)
│   └── helpers.py               # 캡처 파이프라인 헬퍼 (ROI 크롭, 썸네일 등)
│
├── core/
│   ├── app.py                   # OvermaxApp (메인 컨트롤러)
│   ├── game_state.py            # GameSessionState (공유 상태 모델)
│   ├── global_hotkey.py         # Windows 전역 단축키
│   ├── utils.py                 # 유틸리티 (메시지 박스, 환경 검사)
│   └── version.py               # 앱 버전 단일 소스
│
├── data/
│   ├── varchive.py              # V-Archive 곡 DB (songs.json 관리)
│   ├── varchive_client.py       # V-Archive 유저 기록 API 클라이언트
│   ├── varchive_uploader.py     # V-Archive 기록 등록 API 클라이언트
│   ├── record_db.py             # 플레이 기록 로컬 캐시 (SQLite)
│   ├── record_manager.py        # 로컬 DB + V-Archive 캐시 병합 레이어
│   ├── sync_manager.py          # overmax vs V-Archive 비교 및 후보 추출
│   ├── recommend.py             # 유사 난이도 추천
│   ├── app_updater.py           # GitHub Releases 기반 앱 자동패치
│   ├── image_db_updater.py      # GitHub Releases 기반 image_index.db 업데이트
│   └── steam_session.py         # Steam ID 및 세션 관리
│
├── detection/
│   ├── image_db.py              # 재킷 이미지 특징 DB (pHash/HOG/ORB)
│   ├── image_db_cli.py          # ImageDB 관리 CLI 도구
│   ├── play_state.py            # 모드/난이도/Max Combo/Rate 통합 감지
│   ├── ocr.py                   # OCR 감지 로직 (로고, Rate)
│   └── ocr_wrapper.py           # Windows OCR 엔진 래퍼
│
├── overlay/
...
│   ├── utils.py                 # 오버레이 위치 보정 유틸
│   └── ui/
│       ├── header_widget.py     # 오버레이 헤더 위젯
│       ├── footer_widget.py     # 유사 구간 통계 푸터 위젯
