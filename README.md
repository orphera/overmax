...
- 트레이 우클릭 메뉴의 **"설정"**에서 오버레이 투명도를 조절할 수 있습니다.
- 트레이 우클릭 메뉴에서 **"디버그 창"**을 열 수 있습니다.
...
| 설정 파일 | `settings.json` (기본), `settings.user.json` (사용자 변경분) |
| 오버레이 위치 저장 | `settings.user.json` 내 `overlay.position` |
...
- `settings.json`: 배포 시 포함되는 기본 설정 파일입니다. 직접 수정하여 기본값을 변경할 수 있습니다.
- `settings.user.json`: 사용자가 앱 내부(설정창, 드래그 이동 등)에서 변경한 사항이 저장됩니다. `settings.json` 보다 우선순위가 높습니다. Git 추적에서 제외됩니다.
...
overmax/
├── main.py                      # 진입점, 컴포넌트 조립
├── settings.py                  # 설정 로더 (DEFAULT -> settings.json -> user.json 병합)
