...
- [ ] Overlay 전용 로직과 모든 Win32 창에서 재사용 가능한 Infra 로직을 분리한다.
...
- [ ] 트레이, 설정, 동기화, 디버그, 앱 업데이트 진행 알림의 Win32 전환
  요구사항을 최소 수준으로 정리한다.
  ...
  - 앱 업데이트: `app_updater`의 "업데이트 중입니다..." 진행 안내와 업데이트
    중 사용자 입력 제한
- [ ] Infra 범위를 "작은 Win32 toolkit" 수준으로 제한하고, 범용 layout engine이나
  Qt/React식 widget tree는 만들지 않는다.
- [ ] 기존 Win32 overlay가 새 Infra 위에서도 Phase 6 외형과 동작을 유지하는지
  확인한다.
- [ ] PyQt6 보조 창은 그대로 유지하고, 실제 이전 대상과 순서는 다음 Phase에서
  별도로 결정한다.

완료 기준:
- [ ] Overlay가 새 Infra를 사용해도 Phase 6 smoke 기준에서 후퇴하지 않는다.
- [ ] 새 Win32 창을 만들 때 필요한 최소 window/render/text/style 입력 경계가
  문서와 코드에서 드러난다.
- [ ] 트레이, 설정, 동기화, 디버그, 앱 업데이트 진행 알림을 고려한 요구사항은
...
- 현재 `overlay/win32` 책임 목록:
  ...
  - `view_state.py`: `OverlayUpdatePayload`를 Win32 renderer 입력 모델로 변환하는
    overlay 전용 adapter.
  - `back_buffer.py`: memory DC/compatible bitmap/BitBlt 기반 double buffering.
    새 Win32 창에서도 재사용 가능한 Infra 성격이다.
  - `text_renderer.py`: Direct2D DCRenderTarget + DirectWrite 텍스트 렌더러.
    새 Win32 창에서도 재사용 가능한 Infra 성격이다.
- 1차 Infra 분리:
  ...
  - 기존 `back_buffer.py`, `text_renderer.py`는 이름은 유지하되 Phase 7 기준으로
    Infra 모듈로 분류한다. 아직 `style.py`의 theme token과 overlay 좌표는 분리하지
    않았고, saved position 정책도 `Win32OverlayWindow` 안에 남겨 둔다.
