...
impl DetectionPipeline {
    ...
    pub fn process_frame(
        ...
        let state = self.play_state.detect(frame, &self.rois, self.current_song_id);
...
mod tests {
    ...
    fn stays_detecting_until_hysteresis_activates() {
        ...
        let first = pipeline.process_frame(&frame, true, 1.0);
        let second = pipeline.process_frame(&frame, true, 2.0);
        let third = pipeline.process_frame(&frame, true, 3.0);
    ...
    fn resets_state_when_song_select_is_lost() {
        ...
        for idx in 0..3 {
            pipeline.process_frame(&frame, true, idx as f64);
        }
        let output = pipeline.process_frame(&frame, false, 10.0);
