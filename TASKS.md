...
- [ ] PyQt6 메인 오버레이와 Win32 메인 오버레이의 현재 차이 목록화
  - 배경/테두리/투명도, 폰트 크기/굵기, 줄간격, 패딩, 정렬, 긴 텍스트 처리,
    색상, verified/unverified 상태 표현을 비교한다.
- [ ] PyQt6 쪽에서 실제 사용자 기준 룩앤필 원본으로 삼을 요소와 버릴 요소를 결정
  - 단순히 PyQt6를 복제하지 않고, 가독성/성능/캡처 제외 안정성에 영향을 주는
    요소는 Win32 기준으로 재판단한다.
- [ ] Win32 renderer의 스타일 상수를 payload 처리와 분리
  - 색상, spacing, typography, opacity, state color를 한 곳에서 조정할 수 있게
    정리하되, 과한 abstraction은 만들지 않는다.
- [ ] 동일 payload fixture로 PyQt6/Win32 스크린샷 또는 픽셀 기준 비교 경로 추가
  - 가능하면 기존 `test/win32_overlay_smoke.py` 흐름을 확장하고, dev server는
    사용하지 않는다.
- [ ] 긴 제목, 한글/영문 혼합, 낮은 opacity, scale 변경, verified=False 상태를
  대표 케이스로 고정
- [ ] DPI/멀티모니터/scale 변경 시 Win32 레이아웃이 PyQt6 대비 과하게 밀리지
  않는지 확인
- [ ] Win32 opt-in 상태를 유지한 채 실제 앱 smoke로 표시/숨김, 위치 저장,
  noactivate/topmost/capture exclusion이 유지되는지 재확인

완료 기준:
- [ ] 같은 payload에서 PyQt6와 Win32 오버레이가 한 화면 안에서 명확히 다른
  제품처럼 보이지 않는다.
- [ ] Win32 경로의 성능, noactivate/topmost, capture exclusion 방어 동작이
  Phase 5 기준에서 후퇴하지 않는다.
- [ ] 룩앤필 조정은 renderer/style 계층에 한정되고, verified state commit과
  기록 저장 흐름에는 영향이 없다.
